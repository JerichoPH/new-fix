<template>
  <q-select outlined use-input clearable v-model="sourceTypeUuid_alertCreate" :options="sourceTypes_alertCreate"
    :label="labelName" @filter="fnFilter" emit-value map-options
    :display-value="sourceTypesMap[sourceTypeUuid_alertCreate]" />
</template>
<script setup>
...
const sourceTypes_alertCreate = ref([]);
...
const fnFilter = (val, update) => {
  if (val === "") {
    update(() => {
      sourceTypes_alertCreate.value = sourceTypes.value;
    });
    return;
  }

  update(() => {
    sourceTypes_alertCreate.value = sourceTypes.value.filter(
