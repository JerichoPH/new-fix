<template>
  <q-select outlined use-input clearable v-model="organizationStationUuid_alertEdit" :options="organizationStations_search"
    :display-value="organizationStationsMap[organizationStationUuid_alertEdit]" :label="labelName" @filter="fnFilter"
...
const organizationStationUuid_alertEdit = inject("organizationStationUuid_alertEdit");
...
const fnSearch = () => {
  ...
  if (!organizationWorkshopUuid_alertEdit.value) {
    organizationStationUuid_alertEdit.value = "";
    return;
  }
