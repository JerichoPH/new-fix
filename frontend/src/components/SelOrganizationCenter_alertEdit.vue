<template>
  <q-select outlined use-input clearable v-model="organizationCenterUuid_alertEdit"
    :options="organizationCenters_search" :display-value="organizationCentersMap[organizationCenterUuid_alertEdit]"
...
const organizationCenterUuid_alertEdit = inject("organizationCenterUuid_alertEdit");
...
const fnSearch = () => {
  ...
  if (!organizationWorkshopUuid_alertEdit.value) {
    organizationCenterUuid_alertEdit.value = "";
    return;
  }
