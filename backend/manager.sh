...
"start")
    ...
    chmod 755 ./new-fix && ./new-fix -daemon=$daemonParam
    ;;
"stop")
    ...
    ps aux | grep new-fix | grep -v grep | awk '{print $2}' | xargs kill -$killParamCode && echo $killParamText
    ;;
"status")
    # 查看程序运行状态
    ps aux | grep new-fix | grep -v grep | xargs
    ;;
"restart")
...
"build")
    ...
    echo "开始打包……" &&
        ...
        rm -rf "./out/$2" &&
        mkdir -p "./out/$2" &&
        echo "编译『程序：new-fix』『版本：$2』『平台：$goosName』『架构：$goarchCode』" &&
        CGO_ENABLED=0 GOOS=$3 GOARCH=$goarchCode go build -a -race -v -o "./out/$2/" "new-fix" &&
        echo "编译成功" && 
        echo "打包：模板文件" &&
